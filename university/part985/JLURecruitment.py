...
def get_data(date, redis, table_name, s):
    url = 'http://jdjyw.jlu.edu.cn/index.php?r=front/site/calendar_dayEvent&date=' + date.strftime('%Y-%m-%d')
    response = s.get(url)
    content = response.content
    parse_json(content, redis, table_name, date)


def get_jlu_recruitment():
    # 吉林大学
    table_name = 'jlu_company_info'

    redis = jedis.jedis()
    redis.clear_list(table_name)

    session = get_default_session()
    begin_date = datetime.date(2013, 9, 3)
