    ...
    def calculate_num_by_addr(self):
        ...
        for index, value in self.university_company_list_length_dict.items():
            ...
            if university_province in self.p211_up_company_province and university_type in self.p211_up:
                self.p211_up_company_province[university_province] += value
                ...
                self.top_basic_company_province[university_province] += value

            elif university_province not in self.top_basic_company_province and university_province in self.basic_top:
                self.top_basic_company_province[university_province] = value

            if university_type in self.p211_up:
                if university_city in self.p211_up_company_city:
                    self.p211_up_company_city[university_city] += value
                else:
                    self.p211_up_company_city[university_city] = value
            elif university_type in self.basic_top:
                if university_city in self.top_basic_company_city:
                    self.top_basic_company_city[university_city] += value
                else:
                    self.top_basic_company_city[university_city] = value
...
if __name__ == '__main__':
    ...
    print(analysis.p211_up_company_province)
    print(analysis.top_basic_company_province)